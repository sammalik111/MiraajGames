...
const platforms: Platform[] = [
  ...
  { x: 380,  y: GROUND_Y, w: 200,  h: 20, type: "grass" },
  { x: 630,  y: GROUND_Y, w: 120,  h: 20, type: "grass" },
  { x: 800,  y: GROUND_Y, w: 280,  h: 20, type: "grass" },
  { x: 1140, y: GROUND_Y, w: 160,  h: 20, type: "grass" },
  { x: 1380, y: GROUND_Y, w: 240,  h: 20, type: "grass" },
  { x: 1700, y: GROUND_Y, w: 300,  h: 20, type: "grass" },
...
].map((x) => ({ x, y: GROUND_Y - 50, collected: false }));
...
export default function PlatformerGame({ onGameEnd }: GameProps) {
  ...
  // Mirror won/dead/score into refs so the rAF loop reads them without
  // re-creating the effect (which would tear down + restart the game loop).
  const wonRef = useRef(false);
  const deadRef = useRef(false);
  const scoreRef = useRef(0);
  const endedRef = useRef(false);

  const finish = (final: "won" | "dead", msg: string) => {
  ...
  useEffect(() => {
    let raf = 0;
    const step = () => {
      if (wonRef.current || deadRef.current) { raf = requestAnimationFrame(step); return; }
      ...
      for (const e of aliveEnemies) {
        ...
        if (vel.y > 0 && prevBottom <= e.y + 4) {
          ...
          finish("dead", "Hit by an enemy — try again.");
      ...
      if (pos.y > STAGE_HEIGHT + 250) {
        finish("dead", "Fell into a pit — try again.");
        raf = requestAnimationFrame(step);
        return;
      }
      ...
      if (pos.x + PLAYER_W > GOAL.x && pos.x < GOAL.x + GOAL.w && pos.y + PLAYER_H > GOAL.y) {
        // Bonus for reaching the flag.
        scoreRef.current += 500;
        setScore(scoreRef.current);
        finish("won", `You reached the goal! 🎉 Score: ${scoreRef.current}`);
      }
    ...
    // Run once per mount — game flags are read from refs so no teardown stutter.
  }, []);
